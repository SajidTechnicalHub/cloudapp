...
const AzureNetworkSecurityGroups = () => {
  const {
  ...
  const Search = (azureNetworkSecurityGroups) => {
    ...
    return azureNetworkSecurityGroups.filter(
      (row) =>
  ...
  const getAzureNetworkSecurityGroups = async () => {

    const response = await axios.get(`${baseUrl}/azure_network_security_groups/get_azure_network_security_grops`, {
      headers: {
  ...
  const updateAzureNetworkSecurityGroups = async () => {
    setIsLoading(true)
    const response = await fetch(`${baseUrl}/azure_network_security_groups/index`, {
      headers: {
        "Content-Type": "application/json",
        Authorization: localStorage.getItem("token"),
      },
  ...
  return (
    <>
      <TopBar subtitle='Azure / All Network Sec Groups' />
        ...
        <div className="search-container">
          ...
          <div className="referesh-container">
            <span className="referesh-block">
              <FiRefreshCcw onClick={() => updateAzureNetworkSecurityGroups()} />
            </span>
          </div>
        </div>
        {isLoading === true ?
          ...
          <Box sx={{ height: 400, width: '100%' }}>
            <DataGrid
              // rows={Search(azureNetworkSecurityGroups)}
              rows={Search(azureNetworkSecurityGroups)}
              columns={columns}
              pageSize={pageSize}
...
export default AzureNetworkSecurityGroups
